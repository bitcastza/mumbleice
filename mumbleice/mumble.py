...
class MumbleConnector:
    def __init__(
        ...
        self.mumble = pymumble.Mumble(server, username, port, password, reconnect=True)
    ...
    def start(self):
        ...
        self.mumble.callbacks.set_callback(
            pymumble.constants.PYMUMBLE_CLBK_DISCONNECTED, self.restart
        )
    ...
    def restart(self):
        ...
        self.mumble = pymumble.Mumble(
            self.server, self.username, self.port, self.password, reconnect=True
        )
        self.silence_count = 0
        self.start()
    ...
    def set_message_callback(self, fn):
        self.mumble.callbacks.set_callback(
            pymumble.constants.PYMUMBLE_CLBK_TEXTMESSAGERECEIVED, fn
        )
