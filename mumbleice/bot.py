...
class Bot:
    def __init__(self, mumble, icecast, command_prefix, autoconnect=False):
        if BUFFER_DURATION % 10 != 0:
            raise ConfigurationError(
                "BUFFER_DURATION must be 10ms or a multiple thereof"
            )
    ...
    def connect_icecast(self):
        if self.icecast.is_connected:
            ...
            audio_file = (
                files("mumbleice")
                .joinpath("resources")
                .joinpath("streaming_started.wav")
            )
    ...
    def disconnect_icecast(self):
        ...
        if self.icecast.is_connected:
            ...
            audio_file = (
                files("mumbleice")
                .joinpath("resources")
                .joinpath("streaming_stopped.wav")
            )
            with as_file(audio_file) as audio:
                self.mumble.send_audio(audio.resolve())
    ...
    def write_audio(self):
        try:
            ...
            self.logger.warning(
                f"No audio received from Mumble for the last {self.mumble.max_silence} s. Disconnecting from Icecast..."
            )
            self.mumble.send_message(
                f"No audio received for the last {self.mumble.max_silence} s. Disconnecting from Icecast..."
            )
            self.disconnect_icecast()
        except BrokenPipeError:
            self.mumble.send_message(
                "Icecast disconnected unexpectedly. Streaming stopped"
            )
            self.disconnect_icecast()
